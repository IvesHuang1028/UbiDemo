...
public class HttpURLConnection_POST {
    ...
    public String sendHttpURLConnectionPOST() {
        try {
            ...
            hucn.setReadTimeout(5*1000);            //設置讀取超時為5秒
            hucn.setConnectTimeout(10*1000);        //設置連接網路超時為10秒
            //設置輸出入流串
            ...
            e.printStackTrace();
            MyLog.e("text","HttpURLConnection_POST="+e.toString());
        }
        return "";
    }
    public String changeInputStream(InputStream inputStream) {    //將輸入串流轉成字串回傳
        ...
        if (inputStream != null) {        //判斷inputStream是否非空字串
            try {
                ...
                e.printStackTrace();
                MyLog.e("text", "Http_Client.changeInputStream.IOException="+e.toString());
            }
        }
